...
// For icons, install react-native-vector-icons and pick an icon set:
...
const DirectPdfScreen = ({ route }) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // If there's at least one match, jump to it
  ...
  // PDF could not be found or downloaded
  if (!localPdfPath) {
    return (
      <View style={styles.loadingContainer}>
  ...
  // Current snippet (optional) - if you want to show snippet text, you can store it here.
    ...
    <View style={styles.container}>
      ...
      <View style={styles.searchBarContainer}>
        ...
        <TextInput
          ...
          onSubmitEditing={handleSearch} // Optionally trigger search on 'Enter'
        />

        {/* Right-side overlay for arrows & match count */}
        {searchResults.length > 0 ? (
          ...
          // If no results, show a Search button
